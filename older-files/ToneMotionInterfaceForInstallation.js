...
TransportButton.onclick = function() {
  ...
  // when all audio buffers load, #TransportButton gets set to .musicIsReadyToPlay
    ...
    // test to set Transport position to universal synchronized time
    Tone.Transport.position = Math.floor((Date.now()%60000)/1000);
...
function updateStatusLabels() {
  if (ToneMotion.status === "deviceDoesNotReportMotion") {
    ...
    // $("#AccelLabelX").html("x-axis: " + ToneMotion.x.toFixed(4));
    // $("#AccelLabelY").html("y-axis: " + ToneMotion.y.toFixed(4));
  ...
  $("#TimeNow").html(Date.now() + " : " + Date.now()%60000 + " : " + Math.floor((Date.now()%60000)/1000));
}
