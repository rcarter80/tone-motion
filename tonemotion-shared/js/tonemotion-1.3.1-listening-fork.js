...
// NOTE: This alteration of the library is for listening sites to use with classes so it is NOT interactive in any way
...
ToneMotion.prototype.setStatus = function(status) {
  ...
  switch (status) {
    ...
    case 'waitingForPieceToStart':
      ...
      this.publicMessage('No music is playing now, but it will automatically start at the same time for everyone.');
      break;
    case 'playing_tacet':
    ...
    case 'playing_tiltAndShake':
      ...
      break;
    case 'playing_listen':
      this.setStatusLabel('playing', 'default');
      this.setStartStopButton('stop', 'stop');
      break;
    case 'missedCue':
...
ToneMotion.prototype.beginMotionUpdates = function() {
  ...
  if (this.accel.rawX === undefined) {
    var motionTestTimeoutID = setTimeout(() => {
      ...
      if (this.accel.rawX === undefined) {
        // no need for error message with desktop Chrome
        this.testWithoutMotion();
      }
    }, 500);
  }
